...
@Client.on_message(filters.command('start') & filters.private & env.SUDOERS)
async def start(app, msg):
    ...
    if len(msg.command) < 2:
        ...
        await msg.reply(TEXT.format(msg.from_user.mention))
        return

    file_id = msg.command[1]
    msg_ids = DB.get_file_id(file_id)
    ...
    for i in msg_ids:
        try:
            ...
            if message.empty:
                ...
                continue 
            await message.copy(chat_id = msg.chat.id)
